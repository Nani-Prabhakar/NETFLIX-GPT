...
const Header = () => {
  ...
  return (
    <div className='absolute w-screen px-8 py-2 bg-gradient-to-b from-black  z-30 flex  justify-between'>
      ...
      alt='logo'></img>
     <div>
    {gpt && <select className='bg-gray-900 p-2 m-2 text-white' onChange={handleLanguageChange}>
      {SUPPORTED_LANGUAGES.map(lang=><option key={lang.identifier} value={lang.name}>{lang.name}</option>)}
    </select >}
     <button onClick={handleGPTSearch} className=' bg-green-500 p-3  mr-4 px-12 text-white text-lg rounded-lg hover:bg-opacity-80' >{gpt?"Home":"GPT"}</button>
